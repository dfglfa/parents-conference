...
class SlotDAO extends AbstractDAO
{
  ...
  public static function calculateSlots($event, $asDummy = false)
  {
    ...
    $breakTimer = 0;
    $breakCounter = 1;
    $breaks = $event->getBreaks();
    while ($startTime < $event->getDateTo()) {
      $endTime = $startTime + ($slotDuration * 60);
      $type = 1;

      // negative break number indicates "interval breaks", meaning a break
      // after every -break slots. (e.g. -5 means a break after every 5 slots).
      if ($breaks < 0 && $breakCounter > 1 && $breakCounter % (-$breaks) == 1) {
        $type = 2;
      }
